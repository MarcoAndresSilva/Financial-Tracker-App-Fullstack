...
- **Feedback visual en la UI:** al probarlo se notó que un registro exitoso no daba ninguna señal visible antes de redirigir. Primer intento con `MatSnackBar` (descartado, ver Paso 27) por sentirse poco visible; se reemplazó por el `NotificationService` + `AlertDialogComponent` centralizados del Paso 27.
...
- **Objetivo:** el gráfico de "Gastos por Categoría" (`ngx-charts-pie-chart`, doughnut) se veía genérico/desprolijo y la leyenda se solapaba con pocas categorías — no ayudaba a entender rápido en qué se iba la plata. Se reemplazó por algo orientado a comparar magnitud con porcentajes visibles.
- **Decisión (siguiendo la skill de dataviz del proyecto):** un donut es *part-to-whole*, pero la skill marca ese caso como "el donut queda deprioritizado — part-to-whole va en stacked bar". Como el objetivo real es comparar magnitud entre categorías (¿cuál se lleva más plata?), se optó por una **lista de barras horizontales rankeadas** (mayor a menor), con nombre de categoría, barra, monto y porcentaje visibles en cada fila — sin necesidad de leyenda aparte, porque cada barra ya lleva su propia etiqueta directa.
...
- **Objetivo:** primer paso del roadmap "MVP para uso en pareja" (alertas de gasto, metas de ahorro, colaboración en wallet compartida) — se priorizó esta por ser la más rápida de tener funcionando. Mostrar en el dashboard, de un vistazo, si el gasto del mes está bien encaminado respecto a lo que entró, con una cara (🙂/😐/🙁/😡) según el porcentaje.
...
### Paso 36: Metas de Ahorro

- **Objetivo:** siguiente ítem del roadmap "MVP para uso en pareja" — poder definir una meta de ahorro con un monto objetivo concreto (ej. "Viaje a Bariloche: $500.000") y ver el progreso.
- **Decisión de diseño — progreso manual, no calculado automático:** se evaluó calcular el progreso a partir del balance histórico de la wallet (Paso 32), pero se descartó: con más de una meta activa a la vez, el mismo balance no puede "contar" para todas sin duplicarse, y un gasto grande haría bajar el progreso de una meta sin que eso represente la realidad (la plata ahorrada para el viaje no necesariamente se gastó). Se optó por un modelo simple y transparente: cada `SavingsGoal` tiene `targetAmount` (fijo) y `currentAmount` (arranca en 0), y el usuario **aporta manualmente** cuando quiere ("Agregar ahorro" con un monto) — sin ninguna conexión automática con transacciones o el balance.
- **Modelo de datos:** tabla `SavingsGoal` nueva (`walletId`, `name`, `targetAmount`, `currentAmount`, únicos por `(walletId, name)`), con `onDelete: Cascade` desde `Wallet` (migración `add_savings_goal`).
- **Backend — `SavingsGoalModule`** (`savings-goal.controller.ts`, `savings-goal.service.ts`, `dto/`): CRUD estándar (`POST/GET/PATCH/DELETE /savings-goals`) más un endpoint dedicado `POST /savings-goals/:id/contributions` que hace `currentAmount: { increment: amount } }` — nunca se edita `currentAmount` a mano vía el PATCH general, siempre se **suma** un aporte, para que quede claro que es un histórico acumulado y no un valor que se pueda "corregir" arbitrariamente. Mismo criterio de permisos parejos del Paso 35 (cualquier miembro de la wallet puede crear/aportar/editar/borrar, sin distinción OWNER/MEMBER).
- **Frontend:** nueva página `/dashboard/goals` (`SavingsGoalsComponent`, link "Metas de Ahorro" en el sidenav) con una grilla de cards — cada una con nombre, barra de progreso (verde cuando llega al 100%, con un ícono de check), montos actual/objetivo, y botones de "Agregar ahorro", editar y eliminar. Dos diálogos nuevos y chicos: `SavingsGoalFormDialogComponent` (crear/editar: nombre + monto objetivo, mismo patrón `isEditMode` de `TransactionFormComponent`) y `ContributeDialogComponent` (un solo campo: monto a agregar). El borrado reutiliza `ConfirmDialogComponent` tal cual, sin cambios.

### Paso 37: Sidenav Responsive (bug de mobile)

- **Objetivo:** al probar en viewport mobile (simulado, Samsung Galaxy S8+ 360px) se detectó que el sidenav estaba mal configurado para pantallas chicas: `mode="side"` fijo y `isSidenavOpened = true` por defecto sin importar el tamaño de pantalla. En `mode="side"` el sidenav **empuja** el contenido en vez de superponerse — con un ancho fijo de 250px sobre una pantalla de 360px, ocupaba ~70% del ancho permanentemente, y el botón de hamburguesa solo alternaba abierto/cerrado empujando el contenido más, en vez de comportarse como un menú desplegable típico de mobile.
- **Fix — patrón responsive estándar de Angular Material con `BreakpointObserver`** (`@angular/cdk/layout`, ya disponible como dependencia transitiva de Material, sin instalar nada nuevo):
  - `isMobile$ = breakpointObserver.observe(Breakpoints.Handset)` determina si el viewport es de tipo mobile.
  - El `mode` del sidenav ahora es dinámico vía `async` pipe: `'over'` (superpuesto, con backdrop que lo cierra solo al tocar afuera) en mobile, `'side'` (fijo, como antes) en desktop.
  - `isSidenavOpened` **no** se liga directo a `isMobile$` en el template (eso pisaría el toggle manual del usuario en cada ciclo de detección de cambios) — se actualiza vía una suscripción en el constructor cada vez que cambia el breakpoint (abierto en desktop, cerrado en mobile), y sigue siendo un `[(opened)]` de dos vías para que el botón de hamburguesa lo seguir alternando manualmente en cualquier momento.
  - Nuevo `onNavLinkClick(sidenav)`: al tocar un link del menú en mobile (`mode === 'over'`), cierra el sidenav automáticamente — en desktop (`mode === 'side'`) no hace nada, porque ahí el menú debe quedar siempre visible.

### Paso 38: Tres bugs de mobile encontrados al probar en viewport chico

- **Nombre inconsistente en el toolbar:** el toolbar decía "Financial Tracker" en texto plano, mientras el sidenav y el login (desde el Paso 28) usan "FinTrack" + el ícono `savings` como marca. Eran dos nombres de app distintos conviviendo. Se unificó: el toolbar ahora muestra el mismo ícono + "FinTrack" que el resto de la app.
- **FAB de "Añadir Transacción" fuera de pantalla en mobile:** el botón flotante tenía `position: absolute` dentro de un `:host { position: relative; height: 100% }` — pero el contenido real (filtros + lista de transacciones) suele ser más alto que la pantalla, así que "abajo del todo" significaba abajo del contenido completo, no de lo visible. Cambiado a `position: fixed`, que se ancla al viewport sin importar cuánto scroll haya.
- **Panel de filtros ocupando toda la pantalla en mobile:** los 4 filtros apilados verticalmente (Tipo, Rango de Fechas, Categoría, Subcategoría) tapaban la lista de transacciones antes de que se viera ninguna. Se agregó un botón "Filtros" con flecha (`toggleFilters()`) que colapsa/expande el panel — arranca **colapsado en mobile y expandido en desktop** (decidido una sola vez al entrar con `BreakpointObserver.observe(Breakpoints.Handset)`; después el usuario lo maneja a mano con el botón, en cualquier tamaño de pantalla).

### Paso 39: Preparación para Despliegue y PWA

- **Objetivo:** hasta este punto la app solo corría contra `localhost` — para desplegarla en un hosting real y poder instalarla como PWA en un celular hacían falta tres cosas que no existían: URLs de API configurables por entorno, CORS configurable, y el service worker/manifest de una PWA.
- **URL de API centralizada:** los 8 servicios del frontend (`auth`, `dashboard`, `wallet`, `category`, `transaction`, `savings-goal`, `subcategory`, `user`) tenían cada uno su propio `private apiUrl = 'http://localhost:3000'` hardcodeado — un problema real para mobile, porque `localhost` en el teléfono apunta al teléfono mismo, no al servidor. Se creó `src/environments/environment.ts` (dev) y `environment.prod.ts` (prod, con un placeholder de URL a completar cuando se conozca el backend desplegado), con `fileReplacements` en `angular.json` para que el build de producción use el archivo correcto. Los 8 servicios ahora importan `environment.apiUrl` en vez de tener el string propio.
- **CORS configurable + fix de bug:** `main.ts` tenía `origin: 'http://localhost:4200'` fijo y un typo (`Credentials` en mayúscula en vez de `credentials`, que Nest/Express ignoraba silenciosamente en vez de aplicar). Se cambió a `origin: process.env.FRONTEND_URL ?? 'http://localhost:4200'` (así el dominio de producción se configura por variable de entorno sin tocar código) y se corrigió el typo.
- **PWA con el schematic oficial (`ng add @angular/pwa`):** generó `ngsw-config.json`, `public/manifest.webmanifest`, íconos placeholder, y registró `provideServiceWorker(...)` en `app.config.ts` (activo solo fuera de `isDevMode()`, así no interfiere con el desarrollo local). Dos ajustes necesarios después de correrlo:
  - El `npm install` del schematic falló por un choque de versión (`@angular/service-worker` resolvía a `19.2.25`, el resto del proyecto está fijo en `19.2.14`) — se instaló la versión exacta `19.2.14` para mantener todos los paquetes de Angular en la misma línea.
  - El schematic crea los archivos bajo `public/`, una carpeta que este proyecto no tenía configurada en `assets` de `angular.json` (usa la convención más vieja de `src/assets`) — sin el ajuste, el manifest y los íconos quedaban afuera del build final. Se agregó `{ "glob": "**/*", "input": "public", "output": "/" }` a `assets` (la forma de objeto, porque `public` no vive dentro de `sourceRoot` como exige la forma de string simple).
  - `manifest.webmanifest` y el `<title>` de `index.html` tenían el nombre genérico del proyecto Angular ("frontend") en vez de la marca real de la app — se corrigieron a "FinTrack", y se agregó `theme_color`/`background_color` al manifest tomando el verde real del tema (`#026e00`, leído del CSS generado, `--mat-sys-primary`).
  - **Íconos con la marca real:** los 8 archivos generados por el schematic eran el logo genérico de Angular. Se reemplazaron por un ícono propio — mismo glyph "savings" (chanchito) que ya se usa en `mat-icon` en el sidenav/login, sobre el mismo degradé `135deg` verde→violeta (`#026e00` → `#7d00fa`) del `hero-banner` de login/registro (Paso 28/38) — para que el ícono de instalación de la PWA sea consistente con el resto de la marca. Generado con `sharp` (Node) a partir de un SVG compuesto (glyph oficial de Material Symbols + fondo degradado), en los mismos 8 tamaños que ya esperaba `manifest.webmanifest`.
- **Títulos de página en negro plano (bug encontrado probando en el celu):** ningún `h1`/`h2` de página tenía `color` propio — heredaban el negro por defecto del navegador (`transaction-list.component.scss` ni siquiera tenía una regla `h1`). Contra el fondo verde del toolbar y el resto de la identidad visual (Pasos 28/34/38), se sentía apagado y desconectado de la marca. Se agregó `color: var(--mat-sys-primary)` a los títulos principales de cada página (Dashboard, Transacciones, Categorías, Metas de Ahorro) y a los subtítulos de sección ("Este mes"/"Histórico" en el dashboard, "Gastos por Categoría"/"Ingresos por Subcategoría") — mismo verde que ya usa el toolbar y el link activo del sidenav, en vez de negro genérico. También se aplicó al `mat-card-title` de login/registro ("Bienvenido"/"Crear cuenta"), que había quedado afuera de esa pasada.
- **El toolbar nunca tuvo fondo verde de verdad (bug real, no caché):** durante varias vueltas se atribuyó a caché del service worker que el toolbar (menú hamburguesa, marca, saludo, selector de wallet) se viera en negro sobre fondo claro en vez de blanco sobre verde — pero al comparar hashes exactos de los bundles servidos en producción contra un build local recién hecho, se confirmó que el deploy estaba al día: el problema era real. Revisando el código fuente de Angular Material: `color="primary"` en `<mat-toolbar>` no pinta nada por sí solo con la API de theming que usa el proyecto (`mat.theme(...)`, un solo mixin a nivel `html`) — la regla `.mat-toolbar.mat-primary` que da el fondo de color solo se genera si además se incluye `mat.toolbar-theme($theme)` (o el equivalente `all-component-themes`), algo que nunca se agregó. O sea, el toolbar **jamás** tuvo fondo verde real en ningún ambiente — lo que se veía "verde" en pantallas anteriores probablemente eran capturas de estados intermedios o la memoria visual del usuario, no el comportamiento real. Se pintó el fondo y el color de texto a mano con los tokens del tema (`background-color: var(--mat-sys-primary); color: var(--mat-sys-on-primary);` en el propio `mat-toolbar`) — el ícono del menú, el ícono/texto de marca, el saludo y el selector de wallet heredan `color` normalmente (a diferencia de `mat-list-item`, un `mat-icon-button` simple no tiene tokens propios que peleen con la herencia), así que no hizo falta tocarlos aparte. De paso se subió un poco el tamaño de fuente del saludo/selector de wallet (0.95rem → 1.05rem, marca 1.15rem → 1.3rem), a pedido.

### Paso 40: Sesión no se cerraba sola al vencer el JWT

- **Objetivo:** el JWT vence a los 60 minutos en el backend (Paso 3), pero el frontend nunca manejaba ese vencimiento — `authGuard` solo comprueba que *exista* un token en `localStorage`, no que siga siendo válido, y `authInterceptor` solo lo adjuntaba a las peticiones sin reaccionar a un 401. Resultado: dejando la pestaña abierta pasado ese tiempo, la app quedaba en un estado roto en silencio (datos que no cargan, formularios que no guardan) en vez de avisar y volver a login.
- **Decisión — manejar el 401 en el interceptor global, no con un timer de inactividad:** en vez de armar un sistema de detección de inactividad del lado del cliente (más complejidad, y termina duplicando lo que el vencimiento del JWT ya garantiza del lado del servidor), `authInterceptor` ahora intercepta cualquier respuesta 401: si la petición llevaba un token (o sea, no es un intento de login/signup con credenciales incorrectas, que ya maneja su propio formulario), limpia `localStorage`, muestra `NotificationService.error('Tu sesión expiró...')` y redirige a `/auth/login`.
- **Costo aceptado:** al vivir en un interceptor global (registrado desde el arranque de la app, no perezoso), `NotificationService`/`AlertDialogComponent` pasan a formar parte del bundle inicial en vez de cargar solo con las rutas del dashboard — subió el bundle inicial ~63KB, superando el budget de warning de Angular (500KB). Se ajustó el budget a 600KB en `angular.json`: sigue muy lejos del límite real de error (1MB), y el aviso de sesión vencida vale ese costo.

### Paso 41: Español Neutro/Chileno en vez de Voseo Argentino

- **Objetivo:** varios textos de la app (frontend y backend) habían quedado escritos con voseo argentino ("tenés", "podés", "Registrate", "Volvé", "Ingresá", "Pedile", "empezá", etc.) en vez de español neutro latinoamericano/chileno ("tienes", "puedes", "Regístrate", "Vuelve", "Ingresa", "Pídele", "empieza"). Se corrigieron todas las instancias encontradas, tanto en texto de UI (templates, mensajes de `NotificationService`, validaciones) como en mensajes de error del backend que llegan directo a la UI (`WalletService`).
- **Cómo se encontraron:** búsqueda sistemática por patrones de voseo (verbos terminados en `-ás`/`-és`/`-ís` con tilde, imperativos como `-á`/`-é`/`-í` sueltos, y palabras como "vos") en todo `frontend/src` y `backend/src`, filtrando falsos positivos (palabras normales del español que también terminan en vocal acentuada, como "está", "así", "más", "después").
- **Dockerfile no aplicaba migraciones (bug real, no solo de despliegue):** el `CMD` final era `node dist/main` a secas — en un despliegue nuevo, contra una base de datos vacía, la app arrancaría sin ninguna tabla creada. Se copió `prisma/` (schema + migraciones) a la etapa de producción de la imagen, y el `CMD` pasó a `sh -c "npx prisma migrate deploy && node dist/main"` — aplica las migraciones pendientes (nunca genera migraciones nuevas ni pide confirmación, a diferencia de `migrate dev`) antes de arrancar el servidor. Verificado localmente: `docker compose up -d --build` reconstruyó la imagen, corrió `migrate deploy` (detectó las 3 migraciones ya aplicadas, sin cambios) y la app arrancó normal con todas las rutas.
- **Login/Registro con look de "ventana minimizada" en mobile:** la card de login/registro tenía `max-width: 400px` con padding alrededor y bordes redondeados incluso en pantallas de 320-400px de ancho, dejando ver el fondo degradado como un marco — se sentía como una ventana flotando chica en el medio, no como una pantalla de app. Se agregó una media query (`max-width: 600px`) en `login.component.scss` y `register.component.scss` que saca el `max-width`, el padding del contenedor, el `border-radius` y la sombra, y fija la card a una altura fija (probado en vivo desde el inspector del navegador, `height: 85vh` quedó mejor que `100vh` — deja un respiro abajo en vez de pegar la card a los dos bordes) en mobile; en desktop no cambia nada.
  - **Ajuste tras probarlo — el banner del gradiente debe crecer, no quedar fijo:** con la card a pantalla completa, el `.hero-banner` seguía con su padding fijo del diseño de desktop, dejando un espacio vacío entre el gradiente y el borde superior real de la pantalla. Se le agregó `flex: 1` (la card ya es `display: flex; flex-direction: column`) para que el banner absorba todo el espacio sobrante hacia arriba — el formulario abajo mantiene su alto natural, el gradiente ocupa el resto. Dentro de ese espacio más grande, el ícono y el texto de marca también se agrandaron (`hero-icon` 48px→88px, `hero-brand` 1.5rem→2.25rem) para que se sientan proporcionados al nuevo tamaño del banner.

### Paso 42: Eliminar Wallets + Cierre de Sesión por Inactividad

- **Objetivo — eliminar wallets:** con la app en producción y wallets compartidas ya en uso (Paso 35), hacía falta una forma de deshacer una wallet creada por error o que ya no se usa. El riesgo principal es de datos: borrar la wallet equivocada, o dejar a un usuario sin ninguna wallet activa.
- **Reglas de negocio:** solo quien tiene rol `OWNER` puede eliminar la wallet (un `MEMBER` de una wallet compartida no puede eliminarla, solo su creador); no se puede eliminar la única wallet que le queda al usuario (bloqueado con `409 Conflict` antes de tocar la base de datos); si la wallet tiene transacciones registradas, el diálogo de confirmación lo advierte explícitamente en vez de borrar en silencio.
- **Backend (`WalletService`):** `getMyWallets(userId)` devuelve las wallets del usuario junto con su rol y `transactionCount` (vía `_count` de Prisma) — la UI necesita ambos datos para decidir si mostrar el botón de eliminar y con qué advertencia. `deleteWallet(userId, walletId)` verifica la membresía y el rol, cuenta las wallets del usuario para bloquear el caso de "única wallet", y delega el borrado en cascada ya definido en el schema de Prisma (`Wallet → Category → Subcategory` y `Wallet → Transaction`/`SavingsGoal`, todos `onDelete: Cascade`) — no hace falta borrar cada tabla a mano.
- **Frontend:** el menú de selección de wallet en el toolbar (`dashboard-layout`) gana un ícono de basurero junto a cada wallet, visible solo si el usuario es `OWNER` de esa wallet y tiene más de una. Al confirmar (`ConfirmDialogComponent`, con mensaje distinto según `transactionCount`), se llama a `deleteWallet` y se recargan tanto la lista local (`myWallets`) como el estado global (`WalletContextService.loadUserWallets()`) — este último ya reasigna sola la wallet activa a la primera de la lista restante en cada carga, así que no hace falta lógica extra para el caso de haber eliminado la wallet que estaba activa.
- **Objetivo — cierre de sesión por inactividad:** el Paso 40 ya maneja el vencimiento del JWT (60 min) reaccionando a un 401, pero un celular o notebook compartido (el caso de uso principal de la app, entre dos personas) puede quedar con la sesión abierta y visible mucho antes de que el token expire. Se agrega un cierre de sesión del lado del cliente a los 5 minutos sin actividad, más corto que el vencimiento del token.
- **Implementación:** en `DashboardLayoutComponent`, un observable combina los eventos `mousemove`, `keydown`, `click`, `scroll` y `touchstart` sobre `document` (`merge` de RxJS); cada evento reinicia un `timer` de 5 minutos vía `switchMap` (que cancela el timer anterior al llegar un evento nuevo). Si el timer llega a completarse sin que ocurra ningún evento en el medio, se dispara el cierre de sesión con un aviso (`NotificationService`) y redirección a `/auth/login`. Vive en el layout del dashboard (no en un servicio global) porque solo debe correr mientras hay una sesión activa — se limpia con `takeUntil(destroy$)` al destruirse el componente.

### Paso 43: Rediseño de Títulos de Sección e Ícono de Wallet

- **"Dashboard" → "Home":** la sección principal se renombró de "Dashboard Principal" a "Home" (título de página y entrada del menú lateral), con el ícono `home` — un nombre más accesible que "Dashboard" para un uso cotidiano no técnico.
- **Ícono grande junto a cada título de sección:** "Home", "Mis Transacciones", "Categorías" y "Metas de Ahorro" ahora muestran, junto al `<h1>`, el mismo ícono que ya tenían en el menú lateral (`home`, `receipt_long`, `category`, `flag`) pero a 32px y en un verde más claro (`#4caf50`) que el color del texto del título (`--mat-sys-primary`) — refuerza visualmente en qué sección está el usuario.
- **Ícono de wallet en el toolbar:** el ícono junto a "Cartera: {nombre}" pasó de 24px/blanco (se perdía contra el resto del texto del toolbar) a 28px y color café oscuro (`#3e2723`), para que se lea de inmediato como una billetera y no como un ícono genérico más.
- **Experimento de tema descartado:** se probó regenerar la paleta M3 completa de la app (`ng generate @angular/material:m3-theme`) a partir del verde vivo `#4caf50` en vez de `mat.$green-palette`, para que el toolbar y los botones "primary" se vieran más brillantes. Se revirtió: el tono 40 de M3 (el que se usa para fondos sólidos con buen contraste, como el toolbar) sigue saliendo oscuro sin importar el hue de origen, así que el cambio no lograba el efecto buscado y sí desviaba el resto de la paleta (secundarios, error, etc.) de lo ya validado. El tema global se mantiene en `mat.$green-palette`/`mat.$violet-palette`; el verde vivo puntual (íconos de títulos y del sidenav) se sigue aplicando como color literal en los componentes que ya lo necesitaban, mismo patrón que el toolbar desde el Paso 28.
