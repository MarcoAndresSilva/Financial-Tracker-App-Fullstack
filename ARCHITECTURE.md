...
- **PWA con el schematic oficial (`ng add @angular/pwa`):** generó `ngsw-config.json`, `public/manifest.webmanifest`, íconos placeholder, y registró `provideServiceWorker(...)` en `app.config.ts` (activo solo fuera de `isDevMode()`, así no interfiere con el desarrollo local). Dos ajustes necesarios después de correrlo:
  ...
  - **Pendiente manual:** los íconos generados por el schematic son el logo genérico de Angular, no el chanchito de la marca — hay que reemplazar los 8 archivos en `public/icons/` por versiones propias en algún momento antes de publicar de verdad.
